...
do
    ...
    Library.Create = function(Self, Class, Properties)
        local Data = {
            ...
            SafeCall = function(_, Function, ...)
                if type(Library.SafeCall) == "function" then
    ...
    Library.Fade = function(Self, Property, Visibility, IsRawItem)
        ...
        Library.FadeState = Library.FadeState or setmetatable({}, { __mode = "k" })
        local ObjectState = Library.FadeState[Object]
        if not ObjectState then
            ObjectState = {}
            Library.FadeState[Object] = ObjectState
        end
        local PropertyState = ObjectState[Property]
        if not PropertyState then
            PropertyState = { Base = Object[Property], Token = 0 }
            ObjectState[Property] = PropertyState
        elseif not Visibility and Object[Property] < 1 then
            PropertyState.Base = Object[Property]
        end
        PropertyState.Token = PropertyState.Token + 1
        local Token = PropertyState.Token
        local BaseTransparency = PropertyState.Base
        ...
        Library:Connect(NewTween.Completed, function()
            if not Visibility then
                task.wait()
                if PropertyState.Token == Token then
    ...
    Library.FadeDescendants = function(Self, Visibility, Callback)
        Self.FadeToken = (Self.FadeToken or 0) + 1
    ...
    do
        ...
        Library.ChatLogger = function(Self, Params)
            Params = Params or {}
            local Logger = {}
            ...
            local Items = {}
            do
                Items["Chat"] = Library:Create("Frame", {
                    Name = "\0",
                    Parent = Library.Holder.Instance,
                    Visible = Params and Params.Visible == true,
                ...
                Items["Title"] = Library:Create("TextLabel", {
                    ...
                    Parent = Items["Chat"].Instance,
                    TextColor3 = Library.Theme["Accent"],
                    Text = Params.Name or Params.name or "Chat",
                    Size = UDim2.new(0, 0, 0, 15),
                    Position = UDim2.new(0, 10, 0, 6),
                    BackgroundTransparency = 1,
                    TextXAlignment = Enum.TextXAlignment.Left,
            ...
            AlignLoggerToWindow(Items["Chat"].Instance)

            local IsVisible = Params and Params.Visible == true
            local ApplyVisibility = function(IsWindowOpen)
                Items["Chat"].Instance.Visible = IsVisible and IsWindowOpen
            end
            ...
            function Logger:Center()
                local AbsPos = Items["Chat"].Instance.AbsolutePosition
                Items["Chat"].Instance.AnchorPoint = Vector2.new(0, 0)
                task.wait()
                Items["Chat"].Instance.Position = UDim2.new(0, AbsPos.X, 0, AbsPos.Y + GuiInset)
            end
            ...
            Library:RegisterLayout("Chat", {
                Instance = Items["Chat"].Instance
            })
        ...
        Library.Page = function(Self, Params)
            ...
            function Page:RestoreActiveSubPage()
                local ActiveSubPage = nil
                for _, SubPage in Page.Pages do
                    if SubPage.Active then
                        ActiveSubPage = SubPage
                        break
                    end
                end
                ActiveSubPage = ActiveSubPage or Page.Pages[1]
                if ActiveSubPage then
        ...
        Library.Toggle = function(Self, Params)
            ...
            function Toggle:ExtendRange()
                if Toggle.SettingsItem then
                    return Toggle.SettingsItem
                end
                ...
                do
                    ...
                    SettingsItems["ConnectCap"] = Library:Create("Frame", {
                        Name = "\0",
                        Parent = SettingsItems["Connect"].Instance,
                ...
                function Settings:SetOpen(Bool)
                    if Bool then
                        Items["Text"].Instance.AnchorPoint = Vector2.new(0, 0)
                        Items["Text"].Instance.Position = UDim2.new(0, ToggleTextOffset, 0, -2)
                        Items["Indicator"].Instance.AnchorPoint = Vector2.new(0, 0)
                        Items["Indicator"].Instance.Position = UDim2.new(0, 0, 0, 2)
                        Items["SubElements"].Instance.Size = UDim2.new(0, 0, 0, 12)
                        Items["Toggle"].Instance.AutomaticSize = Enum.AutomaticSize.Y
                        SettingsItems["Content"].Instance.Visible = true
                        SettingsItems["ConnectCap"].Instance.Visible = true
                        ...
                        SettingsItems["ConnectCap"].Instance.Visible = false
                        Items["Toggle"].Instance.AutomaticSize = Enum.AutomaticSize.None
                        Items["Toggle"].Instance.Size = UDim2.new(1, 0, 0, 12)
                        Items["Text"].Instance.AnchorPoint = Vector2.new(0, 0.5)
                        Items["Text"].Instance.Position = UDim2.new(0, ToggleTextOffset, 0.5, -2)
                        Items["Indicator"].Instance.AnchorPoint = Vector2.new(0, 0.5)
                        Items["Indicator"].Instance.Position = UDim2.new(0, 0, 0.5, 0)
                        Items["SubElements"].Instance.Size = UDim2.new(0, 0, 1, 0)
            ...
            function Toggle:Settings()
                if Toggle.PopupSettingsItem then
                    return Toggle.PopupSettingsItem
                end
                ...
                function Settingss:SetOpen(Bool)
                    ...
                    if Settingss.IsOpen then
                        ...
                        SettingsItems["ToggleSettings"]:Tween({
                            Position = UDim2.new(0, SettingsButton.AbsolutePosition.X, 0,
                                SettingsButton.AbsolutePosition.Y + SettingsButton.AbsoluteSize.Y + 10 + GuiInset)
                        })
                ...
                Toggle.PopupSettingsItem = setmetatable(Settingss, Library)
                return Toggle.PopupSettingsItem
            end
                ...
                do
                    for _, WidgetData in Library.SettingsWidgets do
                        ...
                        if type(WidgetData.Settings) == "function" then
                            local WidgetSettings = WidgetToggle:ExtendRange()
